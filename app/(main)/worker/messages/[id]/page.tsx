      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-[#0A0D1A]/90 backdrop-blur-md border-b border-white/5 px-4 py-3 flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="relative w-10 h-10 rounded-full overflow-hidden border-2 border-[#1E2538] bg-[#1E2538]"><Image src={conversation?.client?.avatar_url || "/client_avatar.png"} alt="Client" fill className="object-cover" /></div>
            <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-[#4ADE80] rounded-full border-2 border-[#0A0D1A]" />
          </div>
          <div><h2 className="font-bold text-sm">{conversation?.client?.full_name || 'عميل'}</h2><span className="text-[10px] text-[#4ADE80]">متصل الآن</span></div>
        </div>
      </div>

...
export default function OrdersPage() {
  const { activeTab: t, setActiveTab: sT, orders: o, loading: l, updateStatus: uS, updateTracking: uT } = useOrders()
    ...
    <SubPageLayout>
      <div className="px-4 py-6 mb-2">
        ...
        <div className="flex flex-col gap-4">
          {l ? <PageLoader /> : o.length ? o.map((x: any) => <OrderCard key={x.id} order={x} activeTab={t} onUpdateStatus={uS} onUpdateTracking={uT} />) : <Empty />}
        </div>
      </div>
    </SubPageLayout>
  )

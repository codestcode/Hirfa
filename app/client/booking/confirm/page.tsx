...
function ConfirmContent() {
  ...
  const handleConfirm = async () => {
    ...
    const booking = await createBooking({
      worker_id: workerId,
      service_name: profile.profession || 'خدمة',
    ...
    if (booking) {
      ...
      if (user) {
        createNotification(user.id, 'تم تأكيد الحجز', `تم حجز ${profile?.profession || 'خدمة'} بنجاح في ${dateStr}`)
      }
        ...
        <div className="mt-4 bg-[#0F172A]/60 rounded-xl p-4 shadow-[0_4px_12px_rgba(5,11,44,0.08)]">
          <div className="flex items-start gap-4 pb-4 border-b border-white/5">
            ...
            <div className="flex-1">
              <h3 className="text-white text-base font-medium">{profile?.profession || 'خدمة'}</h3>
